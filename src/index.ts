...
async function reconcileDomainNames(client: PiHoleClient, ip: string, options: UpdateDomainNameOptions, logger: Logger) {
    ...
    if(options.traefikFile) {
        ...
        traefikDomains.forEach((d) => domains.add(d));
    }
...
async function main() {
    ...
    if(await traefikFile.exists()) {
        logger.info(`Traefik file already exists at '${path.join(DATA_DIRECTORY, TRAEFIK_RULES_FILENAME)}', reconciling domain names...`);
